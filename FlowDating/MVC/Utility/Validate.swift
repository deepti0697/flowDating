...
class Validate: NSObject {
    ...
    //Validate login form
        func validateLogin(vc:SignupVC) -> Bool {
            if vc.txt_email.text?.trimmingCharacters(in: .whitespaces).count == 0  {
                self.showMessage(message: "Please Enter Email Address")
           
              
                return false
            } else if ( !validation.emailValidation(txtFieledEmail: vc.txt_email.text!) ) {

                self.showMessage(message: "Please Enter Valid Email")
             
                return false

            }
            else if vc.txt_mobile.text?.trimmingCharacters(in: .whitespaces).count == 0  {
                self.showMessage(message: "Please Enter Mobile No.")
...
//            }
    ...
    /// validate Complete Profile loan VC
        func validateCompletePrfoile(vc:CompleteProfile1VC) -> Bool {
            if vc.displayNameTxt.text?.trimmingCharacters(in: .whitespaces).count == 0  {
                self.showMessage(message: "Please Enter Name")
           
              
                return false
            }
            else if vc.txtDob.text?.trimmingCharacters(in: .whitespaces).count == 0  {
                self.showMessage(message: "Please Enter your DOB")
