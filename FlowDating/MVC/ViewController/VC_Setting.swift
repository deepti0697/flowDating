...
class VC_Setting: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    var valueArray = ["Payment Settings","Privacy Settings","Active/Deactivate Account","Terms and Conditions","FAQ","Rate","About us"]
...
//        guard let getTag = sender.view?.tag else { return }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
        ...
        if indexPath.row == 6{
            openViewController(controller: AboutUsViewController.self, storyBoard: .homeStoryboard) { (vc) in
            ...
            openViewController(controller: FQsViewController.self, storyBoard: .homeStoryboard) { (vc) in
                
            }
        }
        else if indexPath.row == 3 {
            openViewController(controller: TermAndConditionViewController.self, storyBoard: .homeStoryboard) { (vc) in
    ...
    func LogoutApi(){
        ...
        ServiceClass.sharedInstance.hitServicesForLogOut(params, completion: { (type:ServiceClass.ResponseType, parseData:JSON, errorDict:AnyObject?) in
...
//        UserDefaults.standard.removePersistentDomain(forName: Bundle.main.bundleIdentifier!)
        openViewController(controller: ViewController.self, storyBoard: .mainStoryBoard) { (vc) in
